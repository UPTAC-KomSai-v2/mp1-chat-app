...
public class User {
    ...
    public List<String[]> getChannelsWithRoles() throws SQLException {
        ...
        FROM Channels c
        LEFT JOIN Channel_Managers cm
               ON c.channel_id = cm.channel_id AND cm.user_id = ?
        LEFT JOIN Channel_Members mem
               ON c.channel_id = mem.channel_id AND mem.user_id = ?
        WHERE mem.channel_id IS NOT NULL;
    """;

        List<String[]> result = new ArrayList<>();
        try (Connection conn = DatabaseUtils.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            // Set the user ID for all 3 placeholders
            ...
            while (rs.next()) {
                ...
                result.add(new String[]{channelId, channelName, userID, role});
            }
        }
        return result;
    }
